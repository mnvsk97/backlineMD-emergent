...
class Task(BaseModel):
    ...
    agent_type: str
    status: Literal[
        "pending", "in_progress", "awaiting_approval", "completed", "rejected"
    ] = "awaiting_approval"
...
async def complete_task(task_id: str, decision: TaskDecision):
    ...
    await db.tasks.update_one({"task_id": task_id}, {"$set": {"status": new_status}})
...
async def get_active_agents():
    agents = await db.agent_executions.find(
        {"status": {"$in": ["running", "queued"]}}, {"_id": 0}
    ).to_list(50)
                    ...
                    await db.agent_executions.update_one(
                        {"execution_id": agent["execution_id"]},
                        {
                            "$set": {
                                ...
                                "duration_seconds": agent.get("duration_seconds", 0)
                                + 2,
                            }
                        },
                    )
...
async def get_patient_notes(patient_id: str):
    notes = (
        await db.notes.find({"patient_id": patient_id}, {"_id": 0})
        .sort("created_at", -1)
        .to_list(50)
    )
    return notes
