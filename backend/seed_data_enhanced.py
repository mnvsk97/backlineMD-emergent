...
BACKOFFICE_STAFF = ["Emily Parker - Insurance Coordinator", "David Kim - Records Manager"]
...
# Document statuses
DOCUMENT_STATUSES = ["uploaded", "ingesting", "ingested", "not_ingested", "approved", "rejected"]

# Consent form statuses
CONSENT_FORM_STATUSES = ["to_do", "sent", "in_progress", "signed"]

# Claim statuses
CLAIM_STATUSES = ["pending", "submitted", "received", "under_review", "approved", "denied"]
...
async def seed_database():
    ...
    for doc_name in ["Medical History Summary.pdf", "Insurance Card Front.pdf", "Insurance Card Back.pdf"]:
        doc_id = str(uuid4())
        await db.documents.insert_one({
            "_id": doc_id,
            "tenant_id": DEFAULT_TENANT,
    ...
    appointment_types = [
        {"type": "initial_consultation", "title": "Initial Consultation", "duration": 60},
    ...
    for i in range(6):
        ...
        appointment = {
            ...
            "location": f"Room {100 + i}" if apt_type["type"] != "telehealth" else "Virtual",
            "status": "scheduled",
            "google_calendar": {
        ...
        print(f"   ✓ {patient_name}: {apt_type['title']} at {starts_at.strftime('%I:%M %p')}")
    print("")
    
    # Create claims with varied statuses
    print("💰 Creating insurance claims with varied statuses...")
    insurance_providers = ["Blue Shield", "Aetna", "UnitedHealthcare", "Cigna", "Medicare", "Medicaid"]
    claim_statuses_varied = ["pending", "submitted", "under_review", "approved", "denied", "received"]
    ...
    for i in range(6):
        ...
        claim = {
            ...
            "service_date": (datetime.utcnow() - timedelta(days=random.randint(10, 45))).strftime("%Y-%m-%d"),
            "submitted_date": (datetime.utcnow() - timedelta(days=random.randint(5, 20))).strftime("%Y-%m-%d"),
