...
async def broadcast_event(tenant_id: str, event_type: str, op: str, doc: dict):
    ...
    # Map operation to action for frontend compatibility
    action_map = {"insert": "created", "update": "updated", "delete": "deleted"}
    action = action_map.get(op, op)

    event_data = {"type": event_type, "action": action, "data": doc}
